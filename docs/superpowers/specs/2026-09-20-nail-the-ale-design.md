...
4. **⚙️ Settings** (header button, not a tab) — Gemini key + model id (default `gemini-3.6-flash` — 2.5 was retired for new keys on first real test; "Test" does a real one-word generation), GitHub owner/repo/token ("Test" reads `beers.json`), and the **questions editor**: add / edit (label, type, options) / delete.
